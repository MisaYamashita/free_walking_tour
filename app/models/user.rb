class User < ApplicationRecord
  ...
  scope :join_tours_from_today, -> (id) { User.find(id).joined_tours.where("date > ?", Date.today)} #今日以降の参加予定ツアー
  scope :plan_tours_from_today, -> (id) { User.find(id).tours.where("date > ?", Date.today)} #今日以降の開催予定ツアー
